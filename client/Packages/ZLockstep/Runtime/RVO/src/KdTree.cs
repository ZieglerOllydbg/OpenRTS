...
namespace ZLockstep.RVO
{
    ...
    internal class KdTree
    {
        ...
        private struct IntPair
        {
             ...
             */
            public static bool operator <(IntPair pair1, IntPair pair2)
            {
         ...
         * tested.</param>
         */
        internal bool queryVisibility(zVector2 q1, zVector2 q2, zfloat radius)
        {
            return queryVisibilityRecursive(q1, q2, radius, obstacleTree_);
        }

        internal int queryNearAgent(zVector2 point, zfloat radius)
        {
            zfloat rangeSq = zfloat.MaxValue;
            int agentNo = -1;
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            {
                agentTree_[node].maxX_ = zMathf.Max(agentTree_[node].maxX_, agents_[i].position_.x);
                agentTree_[node].minX_ = zMathf.Min(agentTree_[node].minX_, agents_[i].position_.x);
                agentTree_[node].maxY_ = zMathf.Max(agentTree_[node].maxY_, agents_[i].position_.y);
                agentTree_[node].minY_ = zMathf.Min(agentTree_[node].minY_, agents_[i].position_.y);
            }
            ...
            if (end - begin > MAX_LEAF_SIZE)
            {
                ...
                zfloat splitValue = new zfloat(0, 5000) * (isVertical ? agentTree_[node].maxX_ + agentTree_[node].minX_ : agentTree_[node].maxY_ + agentTree_[node].minY_);
        ...
        private ObstacleTreeNode buildObstacleTreeRecursive(IList<Obstacle> obstacles)
        {
            ...
            for (int i = 0; i < obstacles.Count; ++i)
            {
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    zfloat j1LeftOfI = zMathf.LeftOf(obstacleI1.point_, obstacleI2.point_, obstacleJ1.point_);
                    zfloat j2LeftOfI = zMathf.LeftOf(obstacleI1.point_, obstacleI2.point_, obstacleJ2.point_);

                    if (j1LeftOfI >= -zMathf.RVO_EPSILON && j2LeftOfI >= -zMathf.RVO_EPSILON)
                    {
                        ++leftSize;
                    }
                    else if (j1LeftOfI <= zMathf.RVO_EPSILON && j2LeftOfI <= zMathf.RVO_EPSILON)
                    {
                        ++rightSize;
                    }
                    else
                    ...
                    if (new IntPair(Math.Max(leftSize, rightSize), Math.Min(leftSize, rightSize)) >= new IntPair(Math.Max(minLeft, minRight), Math.Min(minLeft, minRight)))
                    {
                        break;
                    }
                }
...

            {
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    {
                        /* Split obstacle j. */
                        zfloat t = zMathf.Det(obstacleI2.point_ - obstacleI1.point_, obstacleJ1.point_ - obstacleI1.point_) / zMathf.Det(obstacleI2.point_ - obstacleI1.point_, obstacleJ1.point_ - obstacleJ2.point_);

                        zVector2 splitPoint = obstacleJ1.point_ + t * (obstacleJ2.point_ - obstacleJ1.point_);
        ...
        private void queryAgentTreeRecursive(zVector2 position, ref zfloat rangeSq, ref int agentNo, int node)
        {
            if (agentTree_[node].end_ - agentTree_[node].begin_ <= MAX_LEAF_SIZE)
            {
                for (int i = agentTree_[node].begin_; i < agentTree_[node].end_; ++i)
                {
                    zfloat distSq = zMathf.AbsSq(position - agents_[i].position_);
                    if (distSq < rangeSq)
                    {
                        rangeSq = distSq;
            ...
            {
                zfloat distSqLeft = zMathf.Sqr(zMathf.Max(zfloat.Zero, agentTree_[agentTree_[node].left_].minX_ - position.x)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, position.x - agentTree_[agentTree_[node].left_].maxX_)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, agentTree_[agentTree_[node].left_].minY_ - position.y)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, position.y - agentTree_[agentTree_[node].left_].maxY_));
                zfloat distSqRight = zMathf.Sqr(zMathf.Max(zfloat.Zero, agentTree_[agentTree_[node].right_].minX_ - position.x)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, position.x - agentTree_[agentTree_[node].right_].maxX_)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, agentTree_[agentTree_[node].right_].minY_ - position.y)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, position.y - agentTree_[agentTree_[node].right_].maxY_));
        ...
        private void queryAgentTreeRecursive(Agent agent, ref zfloat rangeSq, int node)
        {
            ...
            {
                zfloat distSqLeft = zMathf.Sqr(zMathf.Max(zfloat.Zero, agentTree_[agentTree_[node].left_].minX_ - agent.position_.x)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, agent.position_.x - agentTree_[agentTree_[node].left_].maxX_)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, agentTree_[agentTree_[node].left_].minY_ - agent.position_.y)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, agent.position_.y - agentTree_[agentTree_[node].left_].maxY_));
                zfloat distSqRight = zMathf.Sqr(zMathf.Max(zfloat.Zero, agentTree_[agentTree_[node].right_].minX_ - agent.position_.x)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, agent.position_.x - agentTree_[agentTree_[node].right_].maxX_)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, agentTree_[agentTree_[node].right_].minY_ - agent.position_.y)) + zMathf.Sqr(zMathf.Max(zfloat.Zero, agent.position_.y - agentTree_[agentTree_[node].right_].maxY_));
        ...
        private void queryObstacleTreeRecursive(Agent agent, zfloat rangeSq, ObstacleTreeNode node)
        {
            if (node != null)
            {
                ...
                zfloat agentLeftOfLine = zMathf.LeftOf(obstacle1.point_, obstacle2.point_, agent.position_);
                zfloat obstacleLengthSq = zMathf.AbsSq(obstacle2.point_ - obstacle1.point_);

                if (obstacleLengthSq <= zfloat.Zero)
                {
                    queryObstacleTreeRecursive(agent, rangeSq, node.left_);
                    queryObstacleTreeRecursive(agent, rangeSq, node.right_);
                    return;
                }

                queryObstacleTreeRecursive(agent, rangeSq, agentLeftOfLine >= 0 ? node.left_ : node.right_);

                zfloat distSqLine = zMathf.Sqr(agentLeftOfLine) / obstacleLengthSq;
         ...
         * <param name="node">The current obstacle k-D node.</param>
         */
        private bool queryVisibilityRecursive(zVector2 q1, zVector2 q2, zfloat radius, ObstacleTreeNode node)
        {
            if (node == null)
            {
                return true;
            ...
            zfloat q1LeftOfI = zMathf.LeftOf(obstacle1.point_, obstacle2.point_, q1);
            zfloat q2LeftOfI = zMathf.LeftOf(obstacle1.point_, obstacle2.point_, q2);
            zfloat obstacleLengthSq = zMathf.AbsSq(obstacle2.point_ - obstacle1.point_);
            ...
            zfloat invLengthI = 1 / obstacleLengthSq;

            if (q1LeftOfI >= 0 && q2LeftOfI >= 0)
            {
                return queryVisibilityRecursive(q1, q2, radius, node.left_) && ((zMathf.Sqr(q1LeftOfI) * invLengthI >= zMathf.Sqr(radius) && zMathf.Sqr(q2LeftOfI) * invLengthI >= zMathf.Sqr(radius)) || queryVisibilityRecursive(q1, q2, radius, node.right_));
            }

            if (q1LeftOfI <= 0 && q2LeftOfI <= 0)
            {
                return queryVisibilityRecursive(q1, q2, radius, node.right_) && ((zMathf.Sqr(q1LeftOfI) * invLengthI >= zMathf.Sqr(radius) && zMathf.Sqr(q2LeftOfI) * invLengthI >= zMathf.Sqr(radius)) || queryVisibilityRecursive(q1, q2, radius, node.left_));
            }
            ...
            zfloat point1LeftOfQ = zMathf.LeftOf(q1, q2, obstacle1.point_);
            zfloat point2LeftOfQ = zMathf.LeftOf(q1, q2, obstacle2.point_);
            zfloat invLengthQ = 1 / zMathf.AbsSq(q2 - q1);

            return point1LeftOfQ * point2LeftOfQ >= 0 && zMathf.Sqr(point1LeftOfQ) * invLengthQ > zMathf.Sqr(radius) && zMathf.Sqr(point2LeftOfQ) * invLengthQ > zMathf.Sqr(radius) && queryVisibilityRecursive(q1, q2, radius, node.left_) && queryVisibilityRecursive(q1, q2, radius, node.right_);
        }
    }
}
